...
rangeBetaA <- function(state, hierarchical_model, vecchia_approx,
  ...
  #                num_threads = num_threads,
  #                range_X, hierarchical_model, vecchia_approx, state = state)
  #  testGradientRangeBetaA(dens_to_test = proposed_dens, range_beta = new_range_beta,
  #               field_log_var = new_field_log_var, compressed_chol = state$stuff$proposed_compressed_chol,
  #               field = new_field,
  #               grad_to_test = dens_grad_back, sparse_chol = state$stuff$proposed_sparse_chol,
  #               num_threads = num_threads,
...
rangeLogLikS <- function(field, sparse_chol, field_log_var){
  return(
    ...
    - ncol(sparse_chol) * (.5 * field_log_var[1, 1])
  )
}
...
#' Posterior density of the range and field log variance
#' using sufficient parametrization of the latent field
...
#' sufficient case
testGradientRangeBetaS <- function(
...
#' ancillary case
testGradientRangeBetaA <- function(
    range_beta, field_log_var,
    compressed_chol, sparse_chol,
    field,
    grad_to_test, dens_to_test, num_threads,
