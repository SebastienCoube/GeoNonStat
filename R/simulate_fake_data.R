...
createGnsSimulatorParameters <- function(gns_simulator,
  ...
  if (flap_the_gums)
    message("The coefficients in X_coeff have been set to random Normal coefficients")
  ...
  if (flap_the_gums)
    message(
      "Except for the range intercept (range_X_coeff[1,1]), all the coefficients in range_X_coeff are still set to 0"
    )
  ...
  if (!is.null(gns_simulator$range_PP)) {
    ...
    if (!is.null(gns_simulator$range_PP)) {
      ...
      if (flap_the_gums)
        message(
          paste(
            "The first column of range_PP_coeff (who parametrizes brute range) has been filled with independent Normal coefficients with log-variance range_PP_log_var[1]=",
            range_PP_log_var[1]
          )
        )
  ...
  if (!is.null(gns_simulator$noise_PP)) {
    ...
    if (is.null(noise_PP_log_var)) {
      noise_PP_log_var <- -6
      if (flap_the_gums)
        message(paste("noise_PP_log_var has been set to", noise_PP_log_var))
    }
  ...
  if (flap_the_gums)
    message("------------------")
  if (flap_the_gums)
    message("You can disable this message by setting flap_the_gums to FALSE")
...
simulateGnsData <- function(gns_simulator,
  ...
  log_range_field <- X_PP_mult_right(
    ...
    Y = rbind(
      gns_simulator_parameters$range_X_coeff,
      gns_simulator_parameters$range_PP_coeff
    ),
  ...
  log_noise_field <- X_PP_mult_right(
    ...
    vecchia_approx = gns_simulator$vecchia_approx,
    Y = rbind(
      gns_simulator_parameters$noise_X_coeff,
      gns_simulator_parameters$noise_PP_coeff
    ),
  ...
  latent_field <- as.vector(
    exp(.5 * gns_simulator_parameters$field_log_var) * Matrix::solve(sparse_chol, rnorm(nrow(sparse_chol)))
  )
