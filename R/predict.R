...
predict.GeoNonStat <-  function(
  ...
  if(!is.null(new_noise_X) & !is.data.frame(new_noise_X)){stop("new_noise_X must be a data.frame or NULL")}
  if(!identical(
  ...
  if(!identical(
    ...
    stop(paste(c(
      ...
      colnames(object$covariates$range_X$arg)), collapse = ", "))
  }
