...
#' # TODO
#' # TODO
MessyMessyMcmc <- function(covariates,
                           observed_field,
                           hierarchical_model,
                           vecchia_approx,
                           state,
  ...
  for (iter in seq(n_iterations_update)) {
    ...
    if (all(!is.infinite(beta_covmat) & !is.nan(beta_covmat))) {
      if (all(eigen(beta_covmat)$d > 0)) {
        beta_mean <- c(as.vector(state$stuff$sparse_chol %*% (
          centered_field / exp(.5 * state$params$field_log_var)
        )) %*% sparse_chol_X %*% beta_covmat)
    ...
    chol_list <- parallel::mclapply(mc.cores = num_threads, unique(locs_partition), function(cluster_idx) {
      ...
      posterior_precision_subset <-
        Matrix::Diagonal(length(selected_idx),
                         1 / exp(.5 * state$params$field_log_var)) %*%
        posterior_precision_subset %*% Matrix::Diagonal(length(selected_idx),
                                                        1 / exp(.5 * state$params$field_log_var))
    ...
    for (pass in seq(3)) {
      for (cluster_idx in unique(locs_partition)) {
        ...
        additional_mean_from_field <-
          as.vector((1 / exp(.5 * state$params$field_log_var)) *
                      Matrix::crossprod(
        ...
        state$params$field[selected_idx] <- as.vector(Matrix::t(chololo$P) %*%
                                                        Matrix::solve(
                                                          Matrix::t(chololo$L),
    ...
    for (idx_for_field_logvar in seq(4)) {
      for (field_log_var_idx in seq(2)) {
        ...
        current_U <-
          (
            -beta_prior_log_dens(
              beta = as.matrix(state$params$field_log_var),
              n_PP = 0,
              log_scale = 0,
              beta0_mean = hierarchical_model$scale$beta0_mean,
              beta0_var = hierarchical_model$scale$beta0_sd^2
            ) # normal prior+.5 * sum((state$stuff$lm_residuals - state$params$field[vecchia_approx$locs_match])^2 / state$stuff$noise_var
          ) # observation ll
        )
...
proposed_U <-
  (
    -beta_prior_log_dens(
      beta = as.matrix(new_field_log_var),
      n_PP = 0,
      log_scale = 0,
      beta0_mean = hierarchical_model$scale$beta0_mean,
      beta0_var = hierarchical_model$scale$beta0_sd^2
    ) # normal prior+.5 * sum((state$stuff$lm_residuals - new_field[vecchia_approx$locs_match])^2 / state$stuff$noise_var
  ) # observation ll
)
state$ker_var$field_log_var_ancillary <- update_kernel(
  iter_start = iter_start,
  update_kernel_groupsize = 1,
  kernel_value = state$ker_var$field_log_var_ancillary,
  iter = iter,
  mult = -.25
)
if (current_U - proposed_U > log(runif(1))) {
  ...
  state$params$field <- new_field
        ...
        current_U <-
          (
            -beta_prior_log_dens(
              ...
              beta0_mean = hierarchical_model$scale$beta0_mean,
              beta0_var = hierarchical_model$scale$beta0_sd^2
            ) # normal prior+.5 * fieldT_cholT_chol_field / exp(state$params$field_log_var) # observation ll+vecchia_approx$n_locs * (.5 * state$params$field_log_var) # observation ll
          )
        ...
        proposed_U <-
          (
            -beta_prior_log_dens(
              ...
              beta0_mean = hierarchical_model$scale$beta0_mean,
              beta0_var = hierarchical_model$scale$beta0_sd^2
            ) # normal prior+.5 * fieldT_cholT_chol_field / exp(new_field_log_var) # observation ll+vecchia_approx$n_locs * (.5 * new_field_log_var) # observation ll
          )
        if (current_U - proposed_U > log(runif(1))) {
          state$params$field_log_var <- new_field_log_var
        }
    ...
    if (hierarchical_model$anisotropic) {
      range_reparam_mat <- matrix(c(2, 2, 0, 2, -2, 0, 0, 0, 2 * sqrt(2)), 3)
    }
    L_minus_one <- solve(t(chol(
      solve(covariates$range_X$crossprod_X_locs) / max(solve(
    ...
    for (regime in seq(1 + hierarchical_model$anisotropic)) {
      if (!hierarchical_model$anisotropic)
        hmc_stepsize <- matrix(exp(state$ker_var$range_beta_ancillary))
      if (hierarchical_model$anisotropic &
          regime == 1)
        hmc_stepsize <- diag(c(exp(
          state$ker_var$range_beta_ancillary[1]
        ), 0, 0))
      if (hierarchical_model$anisotropic &
          regime == 2)
        hmc_stepsize <- diag(c(0, rep(
          exp(state$ker_var$range_beta_ancillary[2]), 2
        )))
      ...
      state$momenta$range_beta_ancillary <-
        renew_momentum(state$momenta$range_beta_ancillary, kept_momentum = .9)
      ...
      dens_grad[] <- t(solve(L_minus_one)) %*% (
        ...
        ) # normal prior+X_PP_crossprod(
        X = covariates$range_X$X_locs,
        ...
        Y = # Jacobian of range field wrt range_beta
          t(
            ...
            derivative_sandwiches(
              ...
              left_vector = as.vector(Matrix::solve(
                Matrix::t(state$stuff$sparse_chol),
                -as.vector(
                  vecchia_approx$locs_match_matrix %*% # gradient of  Gaussian observations ll wrt latent field
              ...
              right_vector = state$params$field / exp(.5 * state$params$field_log_var),
              # scaled latent field, the scaling actually belongs to the derivative since the derivative must be scaled
...
for (hmc_step in seq(n_hmc_steps)) {
  ...
  new_field <- exp(.5 * state$params$field_log_var) * as.vector(Matrix::solve(
    new_sparse_chol,
    state$stuff$sparse_chol %*% (
      state$params$field / exp(.5 * state$params$field_log_var)
    )
  ))
    ...
    Y = # Jacobian of range field wrt range_beta
      t(
        ...
        derivative_sandwiches(
          ...
          left_vector = as.vector(Matrix::solve(
            Matrix::t(new_sparse_chol),-as.vector(
              vecchia_approx$locs_match_matrix %*% # gradient of  Gaussian observations ll wrt latent field
          ...
          right_vector = new_field / exp(.5 * state$params$field_log_var),
          # scaled latent field, the scaling actually belongs to the derivative since the derivative must be scaled
          NNarray = vecchia_approx$NNarray,
          sauce_determinant_chef = FALSE,
          num_threads = num_threads
...
state$ker_var$range_beta_ancillary[regime] <- update_kernel(
  ...
  iter_start = iter_start,
  update_kernel_groupsize = 1,
  kernel_value = state$ker_var$range_beta_ancillary[regime],
  mult = -.8
)
...
if (!is.nan(current_U - proposed_U + current_K - proposed_K)) {
  if (log(runif(1)) < current_U - proposed_U + current_K - proposed_K) {
    ...
    for (regime in seq(1 + hierarchical_model$anisotropic)) {
      if (!hierarchical_model$anisotropic)
        hmc_stepsize <- matrix(exp(state$ker_var$range_beta_sufficient))
      if (hierarchical_model$anisotropic &
          regime == 1)
        hmc_stepsize <- diag(c(exp(
          state$ker_var$range_beta_sufficient[1]
        ), 0, 0))
      if (hierarchical_model$anisotropic &
          regime == 2)
        hmc_stepsize <- diag(c(0, rep(
          exp(state$ker_var$range_beta_sufficient[2]), 2
        )))
      q[] <- L_minus_one %*% (state$params$range_beta)
      # initializing momentum
      state$momenta$range_beta_sufficient <-
        renew_momentum(state$momenta$range_beta_sufficient, kept_momentum = .9)
      ...
      dens_grad[] <- t(solve(L_minus_one)) %*% (
        ...
        # normal prior derivative+X_PP_crossprod(
        X = covariates$range_X$X_locs,
        PP = hierarchical_model$range$PP,
        permutate_PP_to_obs = FALSE,
        vecchia_approx = vecchia_approx,
        Y = # Jacobian of range field wrt range_beta
          t(
            ...
            derivative_sandwiches(
              vecchia = state$stuff$compressed_chol,
              # derivative of the (unscaled) NNGP factor
              left_vector = as.vector(state$stuff$sparse_chol %*% (
                state$params$field / exp(.5 * state$params$field_log_var)
              )),
              # left vector = whitened latent field
              right_vector = state$params$field / exp(.5 * state$params$field_log_var),
              # scaled latent field, the scaling actually belongs to the derivative since the derivative must be scaled
              NNarray = vecchia_approx$NNarray,
    ...
    Y = # Jacobian of range field wrt range_beta
      t(
        ...
        derivative_sandwiches(
          vecchia = new_compressed_sparse_chol,
          # derivative of the (unscaled) NNGP factor
          left_vector = as.vector(new_sparse_chol %*% (
            state$params$field / exp(.5 * state$params$field_log_var)
          )),
          # left vector = whitened latent field
          right_vector = state$params$field / exp(.5 * state$params$field_log_var),
...
- sum(log(
  Matrix::diag(state$stuff$sparse_chol)
))
)
proposed_U <-
  (
...
state$ker_var$range_beta_sufficient[regime] <- update_kernel(
  ...
  iter_start = iter_start,
  update_kernel_groupsize = 1,
  kernel_value = state$ker_var$range_beta_sufficient[regime],
  mult = -.8
)
  ...
  if (log(runif(1)) < current_U - proposed_U + current_K - proposed_K) {
    ...
    if (!is.null(hierarchical_model$range$PP)) {
      ...
      for (i in seq(n_range_log_scale_update)) {
        ...
        q <- state$params$range_log_scale + rnorm(
          1 + hierarchical_model$anisotropic,
          0,
          exp(state$ker_var$range_log_scale_sufficient)
        )
        
        new_range_beta <- state$params$range_beta
        new_range_beta[-seq(covariates$range_X$n_regressors), ] <- new_range_beta[-seq(covariates$range_X$n_regressors), ] %*%
          diag(
            exp(-.5 * state$params$range_log_scale[c(1, rep(2, 2 * hierarchical_model$anisotropic))]),
        ...
        current_U <-
          (sum((
            state$params$range_log_scale - hierarchical_model$range$log_scale_bounds[1]
          )^2
          )
          + .5 * sum((
            state$stuff$sparse_chol %*% (
              state$params$field / exp(state$params$field_log_var / 2)
            )
          )^2)
          - sum(log(state$stuff$compressed_chol[1, 1, ])))
        proposed_U <-
          (sum((
            q - hierarchical_model$range$log_scale_bounds[1]
          )^2)
          + .5 * sum((
            new_sparse_chol %*% (
              state$params$field / exp(state$params$field_log_var / 2)
        ...
        state$ker_var$range_log_scale_sufficient <- update_kernel(
          iter = iter,
          iter_start = iter_start,
          update_kernel_groupsize = 3,
          kernel_value = state$ker_var$range_log_scale_sufficient,
          mult = -.25
        )
        
        if ((all(q < hierarchical_model$range$log_scale_bounds[2])) &
      ...
      for (i in seq(n_range_log_scale_update)) {
        ...
        new_sparse_chol <- decompress_chol(vecchia_approx = vecchia_approx, new_compressed_sparse_chol)
        new_field <- as.vector(Matrix::solve(
          new_sparse_chol,
        ...
        current_U <-
          (
            ...
            + .5 * sum((state$stuff$lm_residuals - state$params$field[vecchia_approx$locs_match])^2 / state$stuff$noise_var
            ) # observation ll
          )
        proposed_U <-
          (
            ...
            + .5 * sum((state$stuff$lm_residuals - new_field[vecchia_approx$locs_match])^2 / state$stuff$noise_var
            ) # observation ll
          )
    ...
    dens_grad <- (
      -beta_prior_log_dens_derivative(
        ...
        n_PP = hierarchical_model$noise$PP$n_knots,
      ...
      Y =
        (
          +.5 # determinant part of normal likelihood-(squared_residuals / state$stuff$noise_var) / 2 # exponential part of normal likelihood
        )
    )
    )
# Make a half step for momentum at the beginning
...
for (hmc_step in seq(n_hmc_steps)) {
  ...
  dens_grad <- (
    ...
    Y =
      (
        +.5 # determinant part of normal likelihood-(squared_residuals / new_noise_var) / 2 # exponential part of normal likelihood
      )
  )
  )
...
current_U <-
  (
    -beta_prior_log_dens(
      ...
      beta0_var = hierarchical_model$noise$beta0_sd^2,
      log_scale = state$params$noise_log_scale
    ) # normal prior+.5 * sum(log(state$stuff$noise_var)) # det+.5 * sum(squared_residuals / state$stuff$noise_var) # observations
  )
...
proposed_U <-
  (
    -beta_prior_log_dens(
      ...
      beta0_var = hierarchical_model$noise$beta0_sd^2,
      log_scale = state$params$noise_log_scale
    ) # normal prior+.5 * sum(log(new_noise_var)) # det+.5 * sum(squared_residuals / new_noise_var) # observations
  )
proposed_K <- sum(p^2) / 2


state$ker_var$noise_beta_mala <- update_kernel(
  iter = iter,
  iter_start = iter_start,
  update_kernel_groupsize = 1,
  ...
  for (i in seq(4))
  {
    ...
    if (!is.nan(dens_ratio)) {
      if (dens_ratio > log(runif(1))) {
        if ((new_noise_log_scale > hierarchical_model$noise$log_scale_bounds[1]) &
          ...
          state$ker_var$noise_log_scale <- update_kernel(
            ...
            iter_start = iter_start,
            update_kernel_groupsize = 1,
            kernel_value = state$ker_var$noise_log_scale,
            mult = 1
          )
        }
      }
...
run_parallel_version_goret <- function(geo_non_stat,
                                       n_chains_in_parallel = NULL,
                                       n_threads_per_chain = 10,
                                       n_iterations = 100,
                                       seed = 1) {
  if (is.null(n_chains_in_parallel))
    n_chains_in_parallel <- length(geo_non_stat$states)
  iter_start <- length(geo_non_stat$records$chain_1)
  cl <- parallel::makeCluster(n_chains_in_parallel)
  parallel::clusterExport(
    cl = cl,
    varlist = c("iter_start", "seed", "geo_non_stat", "n_iterations"),
    envir = environment()
  )
  res <- parallel::parLapply(cl = cl, X = seq(length(geo_non_stat$states)), function(chain_idx) {
    MessyMessyMcmc(
      covariates = geo_non_stat$covariates,
      observed_field = geo_non_stat$observed_field,
      hierarchical_model = geo_non_stat$hierarchical_model,
      vecchia_approx = geo_non_stat$vecchia_approx,
