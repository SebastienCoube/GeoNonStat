...
createPP = function(vecchia_approx, matern_range = NULL, knots = NULL, seed=1234, plot=TRUE){
  ...
  if (!is.null(knots) && is.numeric(knots) && is.vector(knots)==1 && any(knots <= 0)) 
  ...
  if(is.null(matern_range)){
    ...
    message(paste("Matern range set to ", signif(matern_range, 3))," 25 % of the space pseudo-diameter")
  }
...
summary.PP <- function(object, ...) {
  cat("Object of class \'PP\' with", 
      ...
      "matern range =", object$matern_range)
