...
# # ### # #Run MCMC chain for 40 iterations
  ...
  samples = runOneChainMcmc(
   ...
   state = state, n_iterations = 500, num_threads = 8, iter_start = 300
  )
#  
...
new_locs = as.matrix(expand.grid(seq(0, 1, .01), seq(0, 1, .01))) 
new_locs = rbind(new_locs, new_locs[rev(seq_len(nrow(new_locs))),])
new_locs = rbind(new_locs, observed_locs[seq(100),])
new_X = as.data.frame(
...
prediction <- predict.GeoNonStat(
  ...
  new_noise_X = new_X, num_threads = 5, new_range_X = NULL, burn_in = .2
)
