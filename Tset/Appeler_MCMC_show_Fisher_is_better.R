...
load("Tset/run_high_noise_Fisher.RData")
MCMC_diags_ = readRDS("Tset/diags_no_fisher.RDS")
plot(
log(MCMC_diags$diags[grep("range", row.names(MCMC_diags$diags)),1]),
log(MCMC_diags_$diags[grep("range", row.names(MCMC_diags_$diags)),1])
)
abline(a=0, b=1)

plot(
log(MCMC_diags$diags[grep("range", row.names(MCMC_diags$diags)),2]),
log(MCMC_diags_$diags[grep("range", row.names(MCMC_diags_$diags)),2])
)
abline(a=0, b=1)
