...
vecchia_approx = createVecchia(observed_locs, m = 6, round_locs = .01)
...
PP_noise = createPP(vecchia_approx, knots = as.matrix(expand.grid(seq(0,1, .025), seq(0,1, .025))), matern_range = .05, reorder_knots = TRUE)
...
plotPointillistPainting(vecchia_approx$locs, fake_data$latent_field, cex= .6, pch= 15)
...
geo_non_stat = GeoNonStatMcmc(
  ...
  n_threads_per_chain = 7, n_iterations = 200, seed = 1)


# plotting and diagnostics
tracePlots(geo_non_stat, keep = "range_beta", burn_in = .0)
tracePlots(geo_non_stat, keep = "field_log_var", burn_in = .0)
tracePlots(geo_non_stat, keep = "range_beta", burn_in = .2)
tracePlots(geo_non_stat, keep = "field_log_var", burn_in = .2)
tracePlots(geo_non_stat, keep = "beta", burn_in = .2)
tracePlots(geo_non_stat, keep = "noise_beta", burn_in = .2)
tracePlots(geo_non_stat, keep = "noise_log_scale", burn_in = .2)
plot(sapply(geo_non_stat$records$chain_1, function(x)mean(x$field)))
