...
timediff1 = mean(sapply(seq(20), function(x){
  t1 = Sys.time()
vecchia_(
  log_range = t(log_range), locs = t(locs), 
  NNarray = t(NNarray), num_threads = 1, compute_derivative = T, 
  smoothness = 1.5, 1,tatato)
  Sys.time()-t1
}))

timediff5 = mean(sapply(seq(20), function(x){
  t1 = Sys.time()
vecchia_(
  log_range = t(log_range), locs = t(locs), 
...
timediff8 = mean(sapply(seq(20), function(x){
  t1 = Sys.time()
vecchia_(
  log_range = t(log_range), locs = t(locs), 
  NNarray = t(NNarray), num_threads = 8, compute_derivative = T, 
...
timediff10 = mean(sapply(seq(20), function(x){
  t1 = Sys.time()
vecchia_(
  log_range = t(log_range), locs = t(locs), 
...
plot(c(1,5,8,10), c(timediff1, timediff5, timediff8, timediff10), ylim = c(0, timediff1))
...
for(sm in smoothness){
  # computing vecchia_ 
  vecchia_(
    log_range = t(log_range), locs = t(locs), NNarray = t(NNarray), num_threads = 1, compute_derivative = T, smoothness = sm, result = tatato, start_idx = 1)
  ...
  log_range_ = log_range 
  log_range_[range_row_idx, range_col_idx] = log_range_[range_row_idx, range_col_idx] + .00001
  vecchia_(log_range = t(log_range_), locs = t(locs), NNarray = t(NNarray), num_threads = 1, compute_derivative = T, smoothness = sm, result = tatato_, start_idx = 1)
